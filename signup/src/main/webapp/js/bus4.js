...
var booked3 = !!localStorage.getItem('booked3') ? $.parseJSON(localStorage.getItem('booked3')) : [];
$(document).ready(function() {
    ...
    sc.get(booked3).status('unavailable');
...
$(function() {
    $('#checkout-button').click(function() {
        ...
        if (Object.keys(booked3).length > 0) {
            Object.keys(booked3).map(k => {
                selected.push(booked3[k])
            })
        }
        localStorage.setItem('booked3', JSON.stringify(selected))
        alert("Seats has been Reserved successfully.")
        location.reload()
    })
    $('#reset-btn').click(function() {
        if (confirm("are you sure to reset the reservation of the bus?") === true) {
            localStorage.removeItem('booked3')
