...
var booked2 = !!localStorage.getItem('booked2') ? $.parseJSON(localStorage.getItem('booked2')) : [];
$(document).ready(function() {
    ...
    sc.get(booked2).status('unavailable');
...
$(function() {
    $('#checkout-button').click(function() {
        ...
        if (Object.keys(booked2).length > 0) {
            Object.keys(booked2).map(k => {
                selected.push(booked2[k])
            })
        }
        localStorage.setItem('booked2', JSON.stringify(selected))
        alert("Seats has been Reserved successfully.")
        location.reload()
    })
    $('#reset-btn').click(function() {
        if (confirm("are you sure to reset the reservation of the bus?") === true) {
            localStorage.removeItem('booked2')
