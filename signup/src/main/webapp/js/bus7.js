...
var booked6 = !!localStorage.getItem('booked6') ? $.parseJSON(localStorage.getItem('booked6')) : [];
$(document).ready(function() {
    ...
    sc.get(booked6).status('unavailable');
...
$(function() {
    $('#checkout-button').click(function() {
        ...
        if (Object.keys(booked6).length > 0) {
            Object.keys(booked6).map(k => {
                selected.push(booked6[k])
            })
        }
        localStorage.setItem('booked6', JSON.stringify(selected))
        alert("Seats has been Reserved successfully.")
        location.reload()
    })
    $('#reset-btn').click(function() {
        if (confirm("are you sure to reset the reservation of the bus?") === true) {
            localStorage.removeItem('booked6')
