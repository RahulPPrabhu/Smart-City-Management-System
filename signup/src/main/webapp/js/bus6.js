...
var booked8 = !!localStorage.getItem('booked8') ? $.parseJSON(localStorage.getItem('booked8')) : [];
$(document).ready(function() {
    ...
    sc.get(booked8).status('unavailable');
...
$(function() {
    $('#checkout-button').click(function() {
        ...
        if (Object.keys(booked8).length > 0) {
            Object.keys(booked8).map(k => {
                selected.push(booked8[k])
            })
        }
        localStorage.setItem('booked8', JSON.stringify(selected))
        alert("Seats has been Reserved successfully.")
        location.reload()
    })
    $('#reset-btn').click(function() {
        if (confirm("are you sure to reset the reservation of the bus?") === true) {
            localStorage.removeItem('booked8')
