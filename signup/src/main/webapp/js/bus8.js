...
var booked7 = !!localStorage.getItem('booked7') ? $.parseJSON(localStorage.getItem('booked7')) : [];
$(document).ready(function() {
    ...
    sc.get(booked7).status('unavailable');
...
$(function() {
    $('#checkout-button').click(function() {
        ...
        if (Object.keys(booked7).length > 0) {
            Object.keys(booked7).map(k => {
                selected.push(booked7[k])
            })
        }
        localStorage.setItem('booked7', JSON.stringify(selected))
        alert("Seats has been Reserved successfully.")
        location.reload()
    })
    $('#reset-btn').click(function() {
        if (confirm("are you sure to reset the reservation of the bus?") === true) {
            localStorage.removeItem('booked7')
