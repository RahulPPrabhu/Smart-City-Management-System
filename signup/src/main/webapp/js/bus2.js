...
var booked1 = !!localStorage.getItem('booked1') ? $.parseJSON(localStorage.getItem('booked1')) : [];
$(document).ready(function() {
    ...
    sc.get(booked1).status('unavailable');
...
$(function() {
    $('#checkout-button').click(function() {
        ...
        if (Object.keys(booked1).length > 0) {
            Object.keys(booked1).map(k => {
                selected.push(booked1[k])
            })
        }
        localStorage.setItem('booked1', JSON.stringify(selected))
        alert("Seats has been Reserved successfully.")
        location.reload()
    })
    $('#reset-btn').click(function() {
        if (confirm("are you sure to reset the reservation of the bus?") === true) {
            localStorage.removeItem('booked1')
